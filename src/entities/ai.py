...
class AI:
    ...
    def minimax(self, board_state, column, depth, alpha, beta, maximizing_player):
        board = self.update_board(board_state, column, maximizing_player)

        if self.terminal_state(board, column, maximizing_player, depth):
        ...
        move_scores = self.sort_moves(board, moves, maximizing_player)

        best_value, best_move = self.search_moves(board, move_scores, depth, alpha, beta, maximizing_player)

        return best_move, best_value

    def update_board(self, board_state, column, maximizing_player):
    ...
    def terminal_state(self, board, column, maximizing_player, depth):
        if depth == 0:
            return True

        if column is not None:
            if maximizing_player and board.check_win('X'):
                return True
            if not maximizing_player and board.check_win('O'):
                return True

        return False

    def sort_moves(self, board, moves, maximizing_player):
    ...
    def search_moves(self, board, move_scores, depth, alpha, beta, maximizing_player):
        if maximizing_player:
            best_value = -1000000
            best_move = None
